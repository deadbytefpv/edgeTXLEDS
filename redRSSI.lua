...
local function fillLeft(ledValue)
  ...
  for i = 1, numLedsTotal, 1 do
    ...
    if ledIndex < numLeds - 1 then
      setRGBLedColor(LED_GROUP1[i], 255, 0, 0)
    elseif ledIndex == numLeds - 1 then
      setRGBLedColor(LED_GROUP1[i], brightness, 0, 0)
    else
...
local function fillRight(ledValue)
  ...
  for i = 1, numLedsTotal, 1 do
    local ledIndex = i - 1
    if ledIndex < numLeds - 1 then
      setRGBLedColor(LED_GROUP2[i], 255, 0, 0)
    elseif ledIndex == numLeds - 1 then
      setRGBLedColor(LED_GROUP2[i], brightness, 0, 0)
    else
